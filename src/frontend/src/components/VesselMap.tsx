...
import { PortTerminalSpec, PORT_TERMINALS, getBerthGeoMap, getVesselGeo } from '../utils/portData';
...
export const VesselMap: React.FC<VesselMapProps> = ({
  ...
  const activePort = currentPort || PORT_TERMINALS[1]; // Default Rotterdam
  const portLat = activePort.lat;
  const portLon = activePort.lon;
  ...
  // Berth geographic coordinates centered around the active port
  const berthGeo = useMemo(() => getBerthGeoMap(activePort), [activePort]);
  ...
  const getPosForVessel = (v: VesselStatusItem, idx: number): [number, number] => {
    return getVesselGeo(v, idx, activePort, berthGeo);
  };

  // Update trail history when vessels or port changes
  useEffect(() => {
  ...
  if (loading && berths.length === 0) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-slate-400 font-medium">Loading {activePort.name} Terminal Radar &amp; Spatial Map...</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl bg-slate-950" style={{ height: 580 }}>
      ...
      <div className="absolute top-3 left-3 z-[1000] bg-slate-900/90 dark:bg-slate-900/95 backdrop-blur-md border border-slate-700/80 rounded-xl px-3 py-2 shadow-2xl flex items-center space-x-2.5 text-xs pointer-events-auto">
        <span className="text-xl">{activePort.flag}</span>
        <div>
          <div className="flex items-center space-x-1.5">
            <p className="font-extrabold text-slate-100 text-xs">{activePort.name}</p>
            <span className="text-[10px] px-1.5 py-0.2 rounded bg-cyan-500/20 text-cyan-300 font-mono font-bold">
              {activePort.id}
            </span>
          </div>
          <p className="text-[10px] text-slate-400 font-mono mt-0.5">
            {portLat.toFixed(4)}°N, {Math.abs(portLon).toFixed(4)}°{portLon >= 0 ? 'E' : 'W'} · {activePort.tradeLane}
          </p>
        </div>
      </div>
